...
def image(data_image):
    ...
    print(gloss)
    if(gloss != "none"):
        translated_text = translator.translate(str(gloss), src= "en", dest= "hi")
        translated_final = " "+translated_text.text
        myobj = gTTS(text=str(gloss), lang="en", slow=False)
        filename = 'D:\\ASL\\web app\\temp.mp3'
        myobj.save(filename)
        ...
        translated_final = ""
    emit('response_back', gloss.upper() + translated_final.upper())
